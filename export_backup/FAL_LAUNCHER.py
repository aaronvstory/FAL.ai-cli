...
class FALLauncher:
    ...
    def launch_web_mode(self):
        """Launch the FastAPI web UI with proper integration"""
        try:
            ...
            try:
                import uvicorn
                from web_app import app  # Import the FastAPI app
            except ImportError as e:
                print(f"{Fore.RED}❌ Error: Missing web dependencies - {e}{Style.RESET_ALL}")
            ...
            # Setup browser opening if enabled
    ...
    def launch_cli_mode(self):
        ...
        try:
            ...
            try:
                ...
                # Fallback to basic main.py with better path handling
                main_py_path = self.script_dir / "main.py"
                if not main_py_path.exists():
                    print(f"{Fore.RED}❌ Error: main.py not found at {main_py_path}{Style.RESET_ALL}")
                    return False
                ...
                # Launch main.py in interactive mode with absolute path
                cmd = [sys.executable, str(main_py_path), "--mode", "interactive"]
                subprocess.run(cmd)
    ...
    def launch_quick_generate(self):
        ...
        try:
            ...
            # Fix model name mapping - main.py expects underscores, not removed
            # Map internal names to CLI argument names
            model_mapping = {
            ...
            cmd = [
                ...
                "--mode", cli_model,
                "--image", image_path,
                "--prompt", prompt,  # Remove extra quotes - subprocess handles this
